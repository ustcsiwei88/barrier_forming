...
int main(){
    ...
    for(auto p: input){
        ...
        for(int i=0; i<obs_num; i++){
            ...
            // if(n_pts == 1){
            //     f >> x >> y;
            //     o.back().pts.emplace_back(x - 1e-2, y - 1e-2);
            //     o.back().pts.emplace_back(x + 1e-2, y - 1e-2);
            //     o.back().pts.emplace_back(x, y + 1e-2);
...
using namespace std::chrono;
    ...
    for(int i=0;i<obj.size();i++){
        for(int j=0;j<obj[i].pts.size();j++){
            ...
            for(int k=0; k<SN; k++){
                double deg = M_PI / SN * k;
                segment seg(obj[i].pts[j], obj[i].pts[j] + vec2(cos(deg) * 1e-2, sin(deg) * 1e-2));
                if(!is_collision(seg)){
                    extend(seg);
                    segments.push_back(seg);
                }else{
                    seg = segment(obj[i].pts[j], obj[i].pts[j] - vec2(cos(deg) * 1e-2, sin(deg) * 1e-2));
                    if(!is_collision(seg)){
                        extend(seg);
                        segments.push_back(seg);
                    }
